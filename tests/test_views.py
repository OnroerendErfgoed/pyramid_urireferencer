...
class ViewTests(unittest.TestCase):
    ...
    def test_is_referenced(self):
        ...
        reg_response_success_ref1 = registry_adapter(
            RegistryResponse(uri, True, False, 0, []), {}
        )
        ...
        # disable afterwards, so that you will have no problems
        # in code that uses that socket module
        httpretty.disable()
        httpretty.reset()
    ...
    def test_get_references_success(self):
        """Test get_references returns a populated RegistryResponse."""
        self.config.registry.settings["urireferencer.registry_url"] = (
            "http://my.registry.org"
        )
        self.config.registry.settings["urireferencer.referencer"] = (
            "test_views.TestReferencerWithResponse"
        )
        ...
        self.assertIsInstance(result, RegistryResponse)
        self.assertEqual(result.query_uri, "http://test.uri/1")
        self.assertTrue(result.success)
        self.assertFalse(result.has_references)
        self.assertEqual(result.count, 0)
        self.assertEqual(result.applications, [])

    def test_get_references_returns_referencer_result(self):
        """Test get_references returns whatever the referencer returns (including None)."""
        self.config.registry.settings["urireferencer.registry_url"] = (
            "http://my.registry.org"
        )
        self.config.registry.settings["urireferencer.referencer"] = (
        ...
        view = ReferencesPluginView(request)
        result = view.get_references()
        # TestReferencer.references returns None - verifying pass-through behavior
...
class TestReferencerWithResponse(Referencer):
    """Referencer that returns a populated RegistryResponse for testing success path."""
